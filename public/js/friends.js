...
function agregarAmigo(friendIdentifier) {
  ...
  if (!friendIdentifier || friendIdentifier.toString().trim() === "") {
    showAlert("danger", "Error: Identificador de usuario no válido")
    return
  }
  ...
  // Deshabilitar botón
  if (button) {
    disableButton(button, "Enviando...")
  }

  // Mostrar indicador de carga
  showAlert("info", "Enviando solicitud de amistad...")
  ...
  fetch("/friends/request", {
    ...
    .catch((error) => {
      ...
      showAlert("danger", error.message || "Error al enviar la solicitud de amistad")
    })
    .finally(() => {
      // Rehabilitar botón
      if (button) {
...
function eliminarAmigo(friendId) {
  if (!friendId) {
    showAlert("danger", "Error: ID de amigo no válido")
    return
  }
  ...
  if (deleteButton) {
    disableButton(deleteButton, "Eliminando...")
  }
  ...
  fetch(`/friends/remove/${friendId}`, {
    ...
    .then((data) => {
      // Mostrar mensaje de éxito
      showAlert("success", data.message || "¡Amigo eliminado correctamente!")
    ...
    .catch((error) => {
      ...
      showAlert("danger", error.message || "Error al eliminar el amigo")
